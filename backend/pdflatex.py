...
class Renderer(backend.BaseRenderer):
    def expand_intermediate(self) -> dict:
        ...
        lang = (getattr(self, "language", None) or "en").lower().split("-")[0]
        ...
        def _parse_date(s: typing.Any) -> datetime.date | None:
            ...
            try:
                if re.fullmatch(r"\d{4}-\d{2}", s):
                    y, m = s.split("-"); return datetime.date(int(y), int(m), 1)
        ...
        def _skill_items_from_ids(ids: list[str]) -> list[dict]:
            ...
            for sid in (ids or []):
                meta = registry.get(str(sid)) or {}
                out.append({"id": sid, "name": tr(meta.get("name") or sid), "level": meta.get("level"), "highlight": False})
            return out
        ...
        for c in (raw.get("classes") or []):
            classes.append({"name": tr(c.get("name")), "provider": tr(c.get("provider")), "year": c.get("year"), "link": c.get("link")})
    ...
    def render(self):
        ...
        def _slugify(s: str) -> str:
            s = str(s)
            return re.sub(r"-{2,}", "-", "".join(ch.lower() if (ch.isalnum() and ch.isascii()) else "-" for ch in s)).strip("-") or "cv"
        ...
        if isinstance(chosen_log, (bytes, bytearray)) and len(chosen_log) > 0:
            ...
            try:
                ...
                pass
            try:
                diag.append(f"Command: {' '.join(cmd) if isinstance(cmd, list) else str(cmd)}")
            except Exception:
                pass
